...
import { ArrowLeft, Plus, Loader2, Volume2, RotateCcw, Check, Sparkles } from 'lucide-react';
...
Choisis des mots courants et utiles pour un débutant.`,
  ...
  if (isGenerating) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <p className="text-sm text-muted-foreground">Génération du vocabulaire...</p>
      </div>
    );
  }
  ...
  if (mode === 'flashcard' && words.length > 0) {
    ...
    return (
      <div className="px-5 pt-14 pb-4 space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => setMode('browse')} className="p-2 rounded-xl hover:bg-muted transition">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div className="flex-1">
            <p className="text-xs text-muted-foreground">{flashcardIndex + 1} / {words.length}</p>
          </div>
        </div>

        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
          <div className="h-full bg-primary rounded-full transition-all duration-500" style={{ width: `${((flashcardIndex + 1) / words.length) * 100}%` }} />
        </div>
        ...
        >
          {!showAnswer ? (
            <>
              <p className="font-arabic text-4xl font-bold" dir="rtl">{word.arabic_word}</p>
              <p className="text-sm text-muted-foreground mt-4">Touchez pour voir la réponse</p>
            </>
          ) : (
            <>
              <p className="font-arabic text-3xl font-bold" dir="rtl">{word.arabic_word}</p>
        ...
        {showAnswer && (
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1 h-12 rounded-2xl" onClick={() => nextFlashcard(false)}>
  ...
  return (
    <div className="px-5 pt-14 pb-4 space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => setSelectedCategory(null)} className="p-2 rounded-xl hover:bg-muted transition">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1">
          <h1 className="text-lg font-bold">{selectedCategory.emoji} {selectedCategory.label}</h1>
      ...
      <div className="space-y-2">
        {words.map((word) => (
          <div key={word.id} className="p-4 rounded-2xl bg-card border border-border flex items-center gap-4">
            <div className="flex-1">
              <p className="font-arabic text-lg font-semibold" dir="rtl">{word.arabic_word}</p>
